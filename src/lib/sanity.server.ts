...
export async function getRelatedPosts(currentId: string, categories: string[]): Promise<BlogPost[]> {
...
// Search
export async function searchContent(searchTerm: string): Promise<any[]> {
...
export async function getContentByType(
  ...
  const [data, total] = await Promise.all([
    client.fetch(`*[_type == "${type}"] | order(_createdAt desc) [${offset}...${offset + limit}]`),
